...
class PlayerNew extends React.Component {
  ...
  handleSubmit = async e => {
    e.preventDefault()
    try {
      const playerData = await axios.post('/api/players', this.state.playerData, {
        headers: { Authorization: `Bearer ${Auth.getToken()}` }
      })
      this.props.history.push(`/players/${playerData.data._id}`)
    } catch (err) {
    ...
    return (
      <>
        ...
        <div className="page-title">
          <h2 id="white-text">Create a player</h2>
        </div>

        <form className="create-page-body" onSubmit={this.handleSubmit}>

          <section className="create-player-card">
            <div className="player-card-outer" id={this.state.playerData.team ? this.teamObject[this.state.playerData.team].color1 : 'create'}>
              <div className="player-card">  
                ...
                <div className="card-image">
                  <div><h1 className="head-outline">👤</h1></div>
                </div>
            ...
            <div className="create-form">
              <div className="create-field">
                ...
                {this.state.playersData
                  ?
                  <select name="team" className="create-dropdown" onChange={this.handleChange} value={this.state.playersData.team}> 
